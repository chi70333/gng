...
| **DB 커넥션** | 함수 인스턴스마다 커넥션 → 폭증 위험. **Neon 풀링 URL** + `connection_limit=15` 또는 **Prisma Accelerate** 사용. |
...
| 키 | 위치 | 메모 |
|---|---|---|
| `DATABASE_URL` | Vercel env (prod/preview/dev 분리) | Neon 풀링 URL, `connection_limit=15` |
...
| `R2_*` | Vercel env | |
| `SENTRY_DSN`, `NEXT_PUBLIC_SENTRY_DSN` | Vercel env | 서버/브라우저 성능·에러 수집 |
| `SENTRY_ORG`, `SENTRY_PROJECT`, `SENTRY_AUTH_TOKEN` | Vercel env | sourcemap 업로드용, token은 production/CI secret |
| `LOG_LEVEL` | Vercel env | production 기본 `info` |
| `QSTASH_TOKEN`, `QSTASH_CURRENT_SIGNING_KEY` | Vercel env | |
| `LEGACY_API_TOKEN` | Vercel env | 외부 API 호환용 |
| `PAYMENT_CALLBACK_TOKEN`, `PAYMENT_START_SECRET` | Vercel env | 결제 콜백/시작 서명 |
...
- Project Settings의 Node.js Version은 `package.json`과 동일하게 `20.x`로 유지한다.
...
vercel logs --environment production --since 6h --status-code 500 --json --no-follow --no-branch
vercel logs --environment production --since 6h --status-code 502 --json --no-follow --no-branch
vercel logs --environment production --since 6h --status-code 503 --json --no-follow --no-branch
vercel logs --environment production --since 6h --status-code 504 --json --no-follow --no-branch
vercel logs --environment production --since 6h --query "Prisma OR timeout OR durationMs OR /api/order OR /api/payment" --json --no-follow --no-branch
```

공개 페이지(`/`, `/category/*`, `/goods/*`)는 반복 요청 시 `x-vercel-cache`가 `MISS`로만 고정되면 안 된다. `private, no-cache, no-store`가 보이면 middleware, Server Action, `cookies()`/`headers()` 사용 여부를 우선 확인한다.
